...
function fixFile(path) {
    ...
    content = content.replace(/aiText\s*=\s*faqMatch\s*\?\s*faqMatch.answer\s*:\s*'[^']+';/g, iText = faqMatch ? faqMatch.answer : 'К сожалению, я не могу ответить на этот вопрос. Пожалуйста, нажмите «Связаться с оператором» для получения помощи от специалиста.';);
    ...
    content = content.replace(/message:\s*'\?\?[^']+'/g, message: 'Вы были переключены на оператора. Пожалуйста, подождите — специалист скоро ответит.');
